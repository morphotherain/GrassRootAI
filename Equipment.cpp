...
std::shared_ptr<GameObject> Equipment::ConvertBasedOnGroupID(UINT groupID)
{
	...
	case 46: {
		return std::make_shared<ThrustEquipments>(objectID);        //采矿激光器
		break;
	}
	case 546: {
		return std::make_shared<MiningImprovementEquipments>(objectID);        //采矿激光器
		break;
	}
...
void Miner::handleTask(const Task& task)
{
	try {
