...
这个项目的开发模式是：在 Mac 上写代码和整理文件，在另一台 Windows 设备上用 Visual Studio / MSVC / DirectX 11 编译测试。后来者不要在这个 Mac 工作区运行 CMake、MSBuild 或其他构建命令；需要构建时请切到 Windows 开发环境，并先补齐 `EVEdata.sqlite`、约 2.3G 的数据库/贴图资源包和其他本地依赖。
