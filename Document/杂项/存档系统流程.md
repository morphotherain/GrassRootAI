...
- [x] MainScene → 进游戏 → SpaceScene
- [x] 游戏中 F10 → 回 MainScene
- [x] 再新建 → 新 slot，不串档
- [x] 删除 `save/initial` 后新建 → 日志 `template file not found`
- [x] `saveSlots.lastPlayedTime` 在加载后更新

### Phase C.6（主界面 UI 闭环）

| 路径 | 操作 |
|------|------|
| 新建 | 主菜单「新游戏」→ 输入名称 → 确认 → SpaceScene |
| 读档 | 主菜单「读取存档」→ 选中 slot → 加载 → SpaceScene |
| 删档 | 存档列表 → 删除 → 确认 → 列表刷新 |
| 快速开始 | 主菜单「快速开始」→ 模板新建并进游戏 |
| 返回 | 游戏中 **F10** → 主菜单；子屏 **Esc** 返回上一级 |
| 游戏内窗口 | 主键盘 **1–5** 或 NumPad **1–5** 打开 Market / Cargo 等 |

> 存档 UI 由 `UINavigationStack` + `resource/ui/*.ui.json` 驱动；`MainScene` 已无 A.4 临时 `UIButton` 回退。
