...
void Transform::LookTo(const XMFLOAT3& targetPosition, const XMFLOAT3& up)
{
	XMVECTOR posVector = XMLoadFloat3(&m_Position);
	XMVECTOR targetVector = XMLoadFloat3(&targetPosition);
	XMVECTOR upVector = XMLoadFloat3(&up);

	// 计算方向向量
	XMVECTOR directionVector = XMVectorSubtract(targetVector, posVector);

	XMMATRIX View = XMMatrixLookToLH(posVector, directionVector, upVector);
	XMMATRIX InvView = XMMatrixInverse(nullptr, View);
	XMFLOAT4X4 rotMatrix;
	XMStoreFloat4x4(&rotMatrix, InvView);
	m_Rotation = GetEulerAnglesFromRotationMatrix(rotMatrix);
