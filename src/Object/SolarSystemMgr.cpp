...
void SolarSystemMgr::Init()
{
	...
	// 注册前先获取实例，确保初始化完成
	auto& instance = SolarSystemMgr::getInstance();
	auto& Taskinstance = TaskMgr::getInstance();
...
void SolarSystemMgr::Update(UINT tick) {
	...
	//for (auto obj : *p_mapObject) {
	//	if (obj.second == nullptr) {
	//		continue;
	//	}
	//	auto base = obj.second->GetComponent<BaseComponent>();
	//	if (base->typeID == 1230 && base->containerID != 0) {
	//		std::shared_ptr<Task> task = std::make_shared<Task>();
	//		task->isInnerTask = true;
	//		task->taskID = 0;
	//		task->publisherId = base->objectID;
	//		task->targetId = base->objectID;
	//		task->taskTypeId = 0;
	//		task->targetSystem = SOLAR_SYSTEM;
	//		(*task->paramsPtr)["destoryObject"] = 1;
...
void SolarSystemMgr::handleTask(Task& task)
{
