...
void dynObjectAttributesManager::removeAttributesByObjectID(int object_id)
{
	DEBUG_("开始删除 ObjectID: {} 的所有属性", object_id);

	// 开启事务
	sqlite3_exec(db, "BEGIN TRANSACTION", nullptr, nullptr, nullptr);

	std::string query = "DELETE FROM dynObjectAttributes WHERE ObjectID = ?";
	...
	if (rc == SQLITE_DONE) {
		DEBUG_("成功删除 ObjectID: {} 的所有属性", object_id);
